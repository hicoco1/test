...
$(document).ready(function(){
	...
	if("${searchFlag_date}" == ""  || "${searchFlag_date}" == 2){
		$(".start_time").hide();
	    $(".end_time").hide();
	}
...
<body>	
<form action="Statistics_main" id="searchForm" name="searchForm" method="POST" >
...
<input type="hidden" name="hour"/>
	...
	<div id="mainFrame" style="width:100%;">
			...
			<table id="mainTable">
				...
				<tr>
					<td colspan="23" style="text-align:center;">검색된 글이 없습니다.</td>
				</tr>
				</c:if>
				...
				<td>
					<c:if test="${data.hour != '' }">
						<fmt:parseDate value="${data.hour}" var="dateFmt" pattern="yyyyMMddHH"/>
     					<fmt:formatDate value="${dateFmt}"  pattern="yyyy-MM-dd HH"/>
					</c:if>
					<c:if test="${data.order_date != ''}">
