...
$(".ckdeleteBtn").click(function(){
		...
		if(confirm("삭제 하시겠습니까?")){
			  ...
			  $(":checkbox[name='chk']:checked").each(function(pi,po){
				...
				$("#mainForm").attr("action","ck_deletebranchoffice_addr_CS").submit();	
			}
    });
	...
	$(".addrupBtn").click(function(){
		if(confirm("적용 하시겠습니까?")){
			var b_id = $("#searchForm").find("input[name=b_id]").val();
				var idx = $(this).parent().find(".idx").val();
			var searchFlag = $("#searchForm").find("select[name=searchFlag]").val();
			var searchContent = $("#searchForm").find("input[name=searchContent]").val();
			var pageNo = $("#searchForm").find("input[name=pageNo]").val();
			...
			$("#mainForm").find("input[name=idx]").val(idx);
			$("#mainForm").find("input[name=b_id]").val(b_id);
			$("#mainForm").find("input[name=action]").val("SETTING");
			...
			$("#mainForm").attr("action","updateversion_CS").submit();	
		}
    });
...
<form action="BranchOffice_addr_CS" name="searchForm" id="searchForm" method="POST" >
		<div id="searchFrame">
			<hr color=#ff0059 />
				...
				지점 주소 관리&nbsp;&nbsp;[&nbsp;지점ID : <font color="#ff0059"><b>${decText}</b></font>&nbsp;]</font>
			<hr color="#ff0059" />
			...
			<table id="searchTable" style="background-color: #b8e2fc; margin-top: 20px;">
				...
				<tr>
					...
					검색&nbsp; :&nbsp; 
						...
						<input type="button" value="지점 주소 수정 적용" class="addrupBtn red small btn" title="지점 주소 버전 업테이드(예 : 1.0.0.1 -> 1.0.0.2)">
				...
				<div id="list"></div>
						...
						<%--
						<input type="hidden" name="sha256_cs" id="sha256_cs" value="${sha256_cs}}">
						 --%>
		</div>
	...
	<input type="hidden"   name="action"/>
		<div id="mainFrame" style="overflow:auto; height: 100%">
			<div><font id="font_little">Total: ${totalCount }</font></div>
			<table id="mainTable"  style="height:100px;">
				<colgroup>
					<col>
					<col>
					<col>
		...
		<div style="width:98%;" id="alwaystop" >
         		<table style="width: 100%;"> 
            		<tr>
               			<td align="left"><input type="button" value="선택삭제" class="ckdeleteBtn red big btn" id="ckdelBtn"></td>
               			<td align="right"><input type="button" value="등록" class="red big btn" id="insertBtn"></td>
            		</tr>
         		</table>
      		</div>   
		</div>
		...
		<div>
			<%@ include file="/WEB-INF/views/cs/cs_popup/addrinsert_popup.jsp"%>
		</div>
		
		<div>
			<%@ include file="/WEB-INF/views/cs/cs_popup/addrupdate_popup.jsp"%>
		</div>
		
		<div>
			<%@ include file="/WEB-INF/views/cs/cs_popup/addrdetail_popup.jsp"%>
		</div>
</body>
</html>