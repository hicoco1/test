...
$(document).ready(function(){
	...
	$(".deleteBtn").click(function(){
		if(confirm("삭제 하시겠습니까?")){
			...
			$("#mainForm").attr("action","deleteheadoffice_citygu").submit();	
		}
    });
...
<form action="HeadOffice_citygu" name="searchForm" id="searchForm" method="POST" >
<input type="hidden"   name="${_csrf.parameterName}" value="${_csrf.token}"/>
	...
	<input type="hidden"   name="action"/>
		...
		<div id="Insertpopup" class="Pstyle">
			...
			<div class="content">
		<form id="InsertForm" name="InsertForm" action="insertheadoffice_citygu" method="POST">
			<input type="hidden"   name="${_csrf.parameterName}" value="${_csrf.token}"/>
			<input type="hidden" name="action"/>
				<table>
					<tr>
						<td id ="pop_header">전국 시/도 시군구 정보 등록</td>
					</tr>
				</table>
				<hr color="#87CEFA" />
				<table id ="pop_layout">
					...
					<tr>
						<td >행정동명 : </td>
						<td><input type="text" name="town_code" value="" maxlength="50"></td>
					</tr>
					</table>
					<hr color="#87CEFA" />
		...
		<div id="Updatepopup" class="Pstyle">
			<span class="b-close">X</span>
			<div class="content">
		<form id="UpdateForm" name="UpdateForm" action="updateheadoffice_citygu" method="POST">
			<input type="hidden"   name="${_csrf.parameterName}" value="${_csrf.token}"/>
			<input type="hidden" name="action">
				<table>
					<tr>
						<td id ="pop_header">전국 시/도 시군구 정보 수정</td>
					</tr>
				</table>
				<hr color="#87CEFA" />
				<table id ="pop_layout">
