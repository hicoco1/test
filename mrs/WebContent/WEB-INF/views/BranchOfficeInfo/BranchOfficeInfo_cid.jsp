...
$(document).ready(function(){
	...
	$(".deleteBtn").click(function(){
		if(confirm("삭제 하시겠습니까?")){
			...
			$("#mainForm").attr("action","deletebranchofficeinfo_cid").submit();	
		}
    });
	
	$(".updateBtn").click(function(){
		var cid_id = $(this).parent().find(".cid_id").val();
		var cid_detail = $(this).parent().find(".cid_detail").val();
		
		$("#UpdateForm").find("input[name=cid_id]").val(cid_id);
		$("#UpdateForm").find("input[name=cid_detail]").val(cid_detail);
		
		$('#Updatepopup').bPopup();
	});
	
	$("#act_insert").click(function(){
		var idx = $("#InsertForm").find("input[name=idx]").val("${maxIdx_Cid}");
		var cid_id = $("#InsertForm").find("input[name=cid_id]").val();
		
		if(cid_id == '') {
			alert("CID 단말기 ID를 입력해주세요.");
			$("#InsertForm").find("input[name=cid_id]").focus();
			return false;
		}
	...
	$("#act_update").click(function(){
		var idx = $("#UpdateForm").find("input[name=idx]").val("${maxIdx_Cid}");
		var searchFlag = $("#searchForm").find("select[name=searchFlag]").val();
		var searchContent = $("#searchForm").find("input[name=searchContent]").val();
		var pageNo = $("#searchForm").find("input[name=pageNo]").val();
		var cid_id = $("#UpdateForm").find("input[name=cid_id]").val();
		
		if(cid_id == '') {
			alert("CID 단말기 ID를 입력해주세요.");
			$("#UpdateForm").find("input[name=cid_id]").focus();
			return false;
		}
...
<form action="BranchOfficeInfo_cid" id="searchForm" name="searchForm" method="POST" >
<input type="hidden"   name="${_csrf.parameterName}" value="${_csrf.token}"/>
				...
				<tr>
					...
					검색&nbsp; :&nbsp; 
						<select name="searchFlag">
							<option value="1">CID 단말기 ID</option>
	...
	<input type="hidden" name="action"/>
			...
			<table id="mainTable">
				...
				<tr class="datalist" style="cursor:pointer">
					...
					<td>
						...
						<input type="hidden" class="cid_detail" value="${data.cid_detail }">
					</td>			
				</tr>
				</c:forEach>
				</c:if>
		...
		<%@ include file="/WEB-INF/views/common/paging.jsp"%>
			...
			<input type="hidden" name="cid_detail" value="">
	...
	<div class="content">
	<form id="InsertForm" name="InsertForm" action="insertbranchofficeinfo_cid" method="POST">
	<input type="hidden"   name="${_csrf.parameterName}" value="${_csrf.token}"/>
	<input type="hidden" name="action"/>
	<table>
		<tr>
			<td id ="pop_header">CID 단말기 정보 등록</td>
		</tr>
	</table>
	<hr color="#87CEFA" />
	<table id ="pop_layout">
		<tr>
			...
			<input type="text" name="cid_id" value="" maxlength="20"></td>
		</tr>
		<tr>
			<td >제조사 및 기기 정보 : </td>
			<td><input type="text" name="cid_detail" value="" maxlength="20"></td>
		</tr>
		</table>
		<hr color="#87CEFA" />
	...
	<div id="Updatepopup" class="Pstyle">
		<span class="b-close">X</span>
		<div class="content">
			<form id="UpdateForm" name="UpdateForm" action="updatebranchofficeinfo_cid" method="POST">
			<input type="hidden"   name="${_csrf.parameterName}" value="${_csrf.token}"/>
			<input type="hidden" name="action"/>
				<table>
					<tr>
						<td id ="pop_header">CID 단말기 정보 수정</td>
					</tr>
				</table>
				<hr color="#87CEFA" />
				<table id ="pop_layout">
					<tr>
						<td >CID 단말기 ID : </td>
						<td><input type="hidden" name="idx" value="">
						<input type="text" name="cid_id" value="" maxlength="20" readonly="readonly"></td>
					</tr>
					<tr>
						<td >제조사 및 기기 정보 : </td>
						<td><input type="text" name="cid_detail" value="" maxlength="20"></td>
